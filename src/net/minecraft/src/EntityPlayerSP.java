...
public class EntityPlayerSP extends EntityPlayer {
	...
	public ArrayList<String> perks = new ArrayList();
	...
	public EntityPlayerSP(Minecraft var1, World var2, Session var3, int var4) {
		...
		this.fetchPerks().thenAccept((var1) -> {
			this.perks.addAll(var1.perks);
		});
	}

	private CompletableFuture<SerialPerks> fetchPerks() {
		Gson var1 = new Gson();
		return CompletableFuture.supplyAsync(() -> {
			return (SerialPerks)var1.fromJson(Utils.httpGET("https://os-mc.net/api/perks/get_perks?username=" + this.username), SerialPerks.class);
		});
	}

	public boolean hasClassicAnimationSP() {
		return this.perks.contains("classic_anim");
	}

	public boolean isStaffSP() {
		return this.perks.contains("staff_menu");
	}

	public boolean isContributorSP() {
		return this.perks.contains("contributor");
	}

	public void moveEntity(double var1, double var3, double var5) {
		if(((Boolean)this.hydraConfig.flight.getValue()).booleanValue() && mod_HydraClient.perks.contains("staff_menu")) {
			boolean var7 = ((Boolean)this.hydraConfig.slowSmoothCamera.getValue()).booleanValue();
			if(var7) {
				super.moveEntity(var1, var3, var5);
				return;
			}

			var1 *= (double)((Integer)this.hydraConfig.flightSpeed.getValue()).intValue();
			var3 *= (double)(((Integer)this.hydraConfig.flightSpeed.getValue()).intValue() / 2);
			var5 *= (double)((Integer)this.hydraConfig.flightSpeed.getValue()).intValue();
		}
	...
	public void onLivingUpdate() {
		if(((Boolean)this.hydraConfig.flight.getValue()).booleanValue() && mod_HydraClient.perks.contains("staff_menu")) {
			boolean var1 = ((Boolean)BetterConfig.getInstance().slowSmoothCamera.getValue()).booleanValue();
			if(this.movementInput.sneak) {
				this.motionY -= var1 ? 0.1D : 0.15D;
	...
	public boolean isSneaking() {
		return mod_HydraClient.SNEAK_TOGGLED || this.movementInput.sneak && !this.sleeping;
	}
